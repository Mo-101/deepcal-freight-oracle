...
import { BrainCog, Lock, Rocket } from "lucide-react";
  ...
  return (
    <header className="w-full flex flex-col border-b border-border bg-gradient-to-r from-blue-900 to-indigo-800 shadow-lg mb-6">
        ...
        <nav className="flex gap-2">
          {navLinks.map(link => (
            <Link
              ...
              to={link.to}
              className={cn(
                "px-4 py-2 rounded-xl text-white font-medium hover:bg-indigo-700 transition-colors duration-150",
      ...
      <div className="text-xs tracking-wide text-indigo-100 flex gap-2 items-center px-8 pb-2">
        <Lock className="w-4 h-4 mr-1 text-yellow-300" />
        <span>
          “Nothing Moves Without the Core.” &ndash; System locked until algorithmic data loaded.
        </span>
      </div>
    </header>
  );
