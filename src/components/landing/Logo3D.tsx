...
export function Logo3D({ isAnimating = false }: Logo3DProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < points.length; i++) {
      for (let j = i + 1; j < points.length; j++) {
        if (points[i].distanceTo(points[j]) < 0.8) {
          ...
          // DeepCAL colors
          const colors = [
            new THREE.Color(0xFFB43A), // solar gold
            new THREE.Color(0xC1FF57), // neon lime
            new THREE.Color(0x00E0C6), // deep aqua
          ];
    ...
    const pointLight1 = new THREE.PointLight(0xFFB43A, 2, 10); // solar gold
    pointLight1.position.set(2, 2, 2);
    scene.add(pointLight1);

    const pointLight2 = new THREE.PointLight(0xC1FF57, 2, 10); // neon lime
